	...
	fs.readdir(
		...
		function (err, files) {
			...
			(function iterator(index) {
				if (index == files.length) {
					var ps;
					// splice fails gracefully if params are out of range
...
function handle_get_album(req, res) {
	...
	var album_name = req.url.substr(7, core_url.length - 12);
	load_album(
		album_name,
		page_num,
		page_size,
 ...
 * just changes the name of the JavaScript laoder that needs to be
 * downloaded.
 */
